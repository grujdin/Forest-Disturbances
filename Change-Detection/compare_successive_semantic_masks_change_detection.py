"""
Compare successive semantic keep/drop masks and identify changes between consecutive observations.
...
# HARD-CODED CONFIG
# =============================================================================
MASK_ROOT = Path("D:/Forest_Disturbance/outputs/semantic_masks_empirical_envelopes_blacklist_and_caution_eligible")
OUTPUT_ROOT = Path("D:/Forest_Disturbance/outputs/semantic_change_detection_blacklist_and_caution_eligible")
SCENE_DIR_DATE_RE = re.compile(r"^20\d{2}-\d{2}-\d{2}$")
...
def discover_scene_dirs(root: Path) -> List[Path]:
    ...
    return sorted(scenes, key=lambda p: pd.Timestamp(p.name))
...
def main() -> None:
    ...
    if len(scene_dirs) < 2:
        raise RuntimeError("Need at least two scene folders to compare successive observations.")
    ...
    _, first_profile = read_mask(scene_dirs[0] / KEEP_MASK_NAME)
    pixel_area_ha = compute_pixel_area_ha_grid(first_profile)

    for pre_dir, post_dir in zip(scene_dirs[:-1], scene_dirs[1:]):
        pre_date = pd.Timestamp(pre_dir.name)
        post_date = pd.Timestamp(post_dir.name)
        pair_name = f"{pre_dir.name}_to_{post_dir.name}"
    ...
    readme.write_text(
        "Successive change detection from semantic keep/drop masks.\n\n"
        "Interpretation:\n"
